    ...
    , m_psfcube_up_fft(nullptr)
{
    ...
    m_psfcube = spread_function_util::create_psf_cube(
                psf,
                lsf,
                m_step[0],
                m_step[1],
    ...
    m_psfcube_u = spread_function_util::create_psf_cube(
                psf,
                lsf,
                m_step_u[0],
                m_step_u[1],
...
const std::vector<int>& SCubeOmp::get_size(void) const
{
    return m_flxcube_size;
}

const std::vector<float>& SCubeOmp::get_step(void) const
{
    return m_step;
}
